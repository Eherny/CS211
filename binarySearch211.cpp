...
#include <iostream>
#include <assert.h>
#include "array.h"//including array.h to print out the array to test if i did
//the swap function correctly
//include assert.h
using namespace std;
...
{
  const int SIZE = 10;
  int ar[SIZE] = {1, 10, 20, 30, 40, 50, 60, 70, 80, 90};//array of numbers
  ...
  assert(binarySearchAscend(ar, SIZE, 100) == -1); //testing if number not found
...
void reverseArray(int a[], int s)
{
  for(int i=0,j= s-1;i<j;i++,j--)//grabs the bigging index and the final index
    swap(a[i],a[j]); //swaps front and ending index's
}
...
int binarySearchAscend(const int a[], int s, int key)
{
  ...
  while(  l<=r )
	...
	r=m-1; //if key is less than the number in index adjust right boundary
      else if(key>a[m])
	l=m+1; //if key is greater than the number in the index adjust left boundary 
    }
  ...
  while( l<=r  )
    {
      ...
      if(a[m]==key) //key was found                                                                                                       
        return m;
      else if(key>a[m]) //if key greater than array at index adjust right side
	r=m-1;
      else if(key<a[m]) //if key less than array at index adjust right side
        l=m+1;
    }
